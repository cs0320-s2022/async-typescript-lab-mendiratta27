...
const suggestions:HTMLElement = document.getElementById("suggestions") as HTMLUListElement
...
document.addEventListener("keyup", async (Event:KeyboardEvent)=>{
  if(Event.key==="a") {
    await updateValues("Virgo", "Taurus", "Aries");
    postAndUpdate();
  }
}
)
